...
DbClient.prototype.deleteNote    = deleteNote;
...
function getAllNotes(cb) {
    var sql = 'SELECT * FROM notes ORDER BY last_modified DESC;';
    var values = null;
    this.runQuery(sql, values, cb);
}
...
function updateNote(note, cb) {
    var sql = "UPDATE notes SET body=$1, last_modified=NOW() WHERE id=$2 RETURNING id";
